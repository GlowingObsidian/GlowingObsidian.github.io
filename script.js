...
const down = '<svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" fill="currentColor" class="bi bi-caret-down" viewBox="0 0 16 16"><path d="M3.204 5h9.592L8 10.481 3.204 5zm-.753.659 4.796 5.48a1 1 0 0 0 1.506 0l4.796-5.48c.566-.647.106-1.659-.753-1.659H3.204a1 1 0 0 0-.753 1.659z"/></svg>'

document.getElementById("main").style.display = "none";
//document.getElementById("info_dialogue").style.display = "none";
...
function nextday() {
  document.getElementById("button_press").play();
  if (dayOfTheWeek == 6) {
    dayOfTheWeek = 0;
  } else {
    dayOfTheWeek += 1;
...
function show_help(){
  if(document.getElementById("info_dialogue").style.display == "none")
  {
    unfade("info_dialogue");
  }
}

function close_help_dialogue(){
  fade("info_dialogue");
}
...
function main(uid, dayDefine) {
    ...
    for (var i = 0; i < coll.length; i++) {
      coll[i].addEventListener("click", function () {
        ...
        if (content.style.maxHeight) {
          ...
          document.getElementById("collapse").play();
        } else {
          document.getElementById("expand").play();
  ...
  if (section === "CSE 1") {
    if (dayOfTheWeek === 0 || dayOfTheWeek === 6) {
      document.getElementById("buttons").innerHTML =
        '<div class="holiday"><p style="text-align: center; margin-top: 50px; color: white; font-size: 30px;">Woohoo! No classes!</p><p><img src="assets/satsun.png" alt="" style="width: 300px; height: 300px; position: relative;"></p></div>';
    } else {
      for (var i = 0; i < CSE1[dayOfTheWeek].length; i++) {
        if (
          ...
          document.getElementById("buttons").innerHTML =
            document.getElementById("buttons").innerHTML +
            `<button class="collapsible" id=but${i}><p>${CSE1[dayOfTheWeek][i].time}<h3>${CSE1[dayOfTheWeek][i].subject}</h3></button><div class="content"><div><p id="classData0">Teacher: ${CSE1[dayOfTheWeek][i].teacher}<p>Books: ${CSE1[dayOfTheWeek][i].books}</p><p>Description: ${CSE1[dayOfTheWeek][i].desc}<p/></p></div></div>`;
          collapse();
        }
      }
    }
